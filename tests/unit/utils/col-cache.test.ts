...
describe("colCache", () => {
  ...
  it("validates addresses properly", () => {
    expect(colCache.validateAddress("A1")).toBeTrue();
    expect(colCache.validateAddress("AA10")).toBeTrue();
    expect(colCache.validateAddress("ABC100000")).toBeTrue();
  ...
  it("gets address structures (and caches them)", () => {
    ...
    expect(colCache.getAddress("D5")).toEqual(addr);
    expect(colCache.getAddress(5, 4)).toEqual(addr);

    addr = colCache.getAddress("E4");
    expect(addr.address).toEqual("E4");
    expect(addr.row).toEqual(4);
    expect(addr.col).toEqual(5);
    expect(colCache.getAddress("E4")).toEqual(addr);
    expect(colCache.getAddress(4, 5)).toEqual(addr);
  });
